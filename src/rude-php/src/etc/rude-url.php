...
class url
{
	...
	public static function param_add($url, $key, $value)
	{
		if (url::param_count($url) == 0)
		{
			return $url . url::param($key, $value, true);
		}

		return $url . url::param($key, $value);
	}

	public static function change_domain($url, $domain)
	{
		$url = parse_url($url);
	...
	public static function clean_scheme($url)
	{
	...
	## [en]: count total query params in the url
	##
	## [example]:
	##
	## $count = url::param_count('http://site.com?first=one&second=two'); # int(2)
	##
	## [return]: int

	/**
	 * @en Count total query params in the url
	 * @ru Возвращает количество параметров в ссылке
	 *
	 * @param string $url
	 *
	 * @return int
	 */
	public static function param_count($url)
	{
		return string::count($url, '?') + string::count($url, '&');
	}

	public static function param($key, $value, $is_first_arg = false)
	{
		$and = '&';
		...
		return $and . urlencode($key) . '=' . urlencode($value);
	}

	/**
	 * @en Check if current page is the website's Home page
	 * @ru Проверяет, является ли текущая страница сайта главной
	 *
	 * @return bool
	 */
